...
Module AutoUpdater
    ...
    Public autoUpdateClose As Boolean = False

    Public Sub UpdateNotifiers()
        If countrecord("tblclientsystemupdate") = 0 Then Exit Sub
        com.CommandText = "select *, date_format(str_to_date(version, '%Y.%m.%d'), '%Y-%m-%d') as dt from tblclientsystemupdate where server=1 and active=1" : rst = com.ExecuteReader
        If rst.Read Then
    ...
    Public Function ExtractSpecificZipFile(ByVal UpdateFileName As String, ByVal version As String) As Boolean
        ...
        Try
            ...
            autoUpdateClose = True
            For i As Integer = My.Application.OpenForms.Count - 1 To 0 Step -1
                If My.Application.OpenForms.Item(i) IsNot frmSystemDownloader Then
