...
Public Class frmStatusResultbyvillage
    ...
    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton1.Click
        ...
        If ckVillage.Checked = False Then
            ...
            If txtMunicipal.Text = "" Then
                If txtArea.Text = "" Then
                    strVillage = " where villcode='" & vilcode.Text & "'"
                Else
                    strVillage = " and villcode='" & vilcode.Text & "'"
                End If
    ...
    Public Sub StatusPreview()
        ...
        If countrecord("tblvoters inner join tblcolor on tblvoters.colorcode=tblcolor.colorcode group by tblvoters.colorcode") <> 0 Then
            ...
            dst = New DataSet
            msda = New MySqlDataAdapter("Select * from tblvillage " & strgeneralqry & " order by villname asc ", conn)
            msda.Fill(dst, 0)
            For cnt = 0 To dst.Tables(0).Rows.Count - 1
                With (dst.Tables(0))
                    Dim BarChart As New ChartControl()
                    com.CommandText = "select count(*) as cnt,(select colorname from tblcolor where colorcode = tblvoters.colorcode) as 'color',tblcolor.description from tblvoters inner join tblcolor on tblvoters.colorcode=tblcolor.colorcode where " _
                                    + " tblvoters.areacode='" & .Rows(cnt)("areacode").ToString() & "' and " _
                                    + " tblvoters.muncode='" & .Rows(cnt)("muncode").ToString() & "' and " _
                                    + " tblvoters.vilcode='" & .Rows(cnt)("villcode").ToString() & "' and deceased=0 group by tblvoters.colorcode order by count(*) desc" : rst = com.ExecuteReader
                    While rst.Read
                    ...
                    End While
                    rst.Close()
                    If countqry("tblvoters inner join tblcolor on tblvoters.colorcode=tblcolor.colorcode", "tblvoters.areacode='" & .Rows(cnt)("areacode").ToString() & "' and tblvoters.muncode='" & .Rows(cnt)("muncode").ToString() & "' and tblvoters.vilcode='" & .Rows(cnt)("villcode").ToString() & "' and deceased=0 group by tblvoters.colorcode order by count(*) desc") <> 0 Then
                        Dim chartTitle1 As New ChartTitle()
                        chartTitle1.Text = .Rows(cnt)("villname").ToString()
