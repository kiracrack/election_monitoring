...
Public Class MainMonitoring
    ...
    Private Sub XtraForm1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        FilterSecurity()
        If LCase(globaluser) = "kiracrack" Then
    ...
    Private Sub BarButtonItem5_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles BarButtonItem5.ItemClick
        SplashScreenManager.ShowForm(GetType(WaitForm1), True, True)
        frmCaptureFromCommelecNew.MdiParent = Me
        frmCaptureFromCommelecNew.Show()
        frmCaptureFromCommelecNew.Focus()
        SplashScreenManager.CloseForm()
    End Sub
    ...
    Private Sub BarButtonItem4_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles cmdCheckMissing.ItemClick
        SplashScreenManager.ShowForm(GetType(WaitForm1), True, True)
        frmMissingCapture.Show()
        SplashScreenManager.CloseForm()
    End Sub
            ...
            For Each message As DecodedShortMessage In messages
                If TypeOf message.Data Is SmsDeliverPdu Then
                    ...
                    If countqry("tblsmsinbox", "smsdatetime='" & ConvertDateTime(data.SCTimestamp.ToString()) & "' and sender='" & data.OriginatingAddress & "' and message='" & rchar(data.UserDataText) & "'") = 0 Then
                        Dim getFullname As String = qrysingledata1("fullname", "fullname", "tblvoters where contactnumber='" & data.OriginatingAddress & "' or replace(contactnumber,'+63','0')='" & data.OriginatingAddress & "'")
                        Dim getarea As String = qrysingledata1("areaname", "areaname", "tblarea where areacode='" & qrysingledata1("areacode", "areacode", "tblvoters where contactnumber='" & data.OriginatingAddress & "' or replace(contactnumber,'+63','0')='" & data.OriginatingAddress & "'") & "'")
                        Dim getCity As String = qrysingledata1("munname", "munname", "tblmunicipality where muncode='" & qrysingledata1("muncode", "muncode", "tblvoters where contactnumber='" & data.OriginatingAddress & "' or replace(contactnumber,'+63','0')='" & data.OriginatingAddress & "'") & "'")
                        Dim getBarangay As String = qrysingledata1("villname", "villname", "tblvillage where villcode='" & qrysingledata1("vilcode", "vilcode", "tblvoters where contactnumber='" & data.OriginatingAddress & "' or replace(contactnumber,'+63','0')='" & data.OriginatingAddress & "'") & "'")
                        Dim getPurok As String = ""
                        Dim getPrecinct As String = qrysingledata1("precinct", "precinct", "tblvoters where contactnumber='" & data.OriginatingAddress & "'  or replace(contactnumber,'+63','0')='" & data.OriginatingAddress & "'")

                        Dim getid As String = createTRNID("I") & "-" & globaluserid
